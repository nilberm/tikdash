...
accountsRouter.post('/:id/reveal', async (c) => {
    ...
    if (!userPassword)
        return c.json({ error: 'User password is required' }, 400);
    ...
    if (!account)
        return c.json({ error: 'Not found' }, 404);
    if (!account.password)
        return c.json({ error: 'No password saved for this account' }, 400);
    try {
...
accountsRouter.post('/:id/metrics', async (c) => {
    ...
    const [existingToday] = await db.select()
        .from(accountMetrics)
        .where(and(eq(accountMetrics.accountId, id), gte(accountMetrics.recordedAt, startOfDay), lte(accountMetrics.recordedAt, endOfDay)));
    let result;
    if (existingToday) {
...
accountsRouter.put('/:id/videos/:videoId', async (c) => {
    ...
    if (tiktokUrl !== undefined)
        updateFields.tiktokUrl = tiktokUrl;
    if (thumbnail !== undefined)
        updateFields.thumbnail = thumbnail;
    if (postedAt !== undefined)
        updateFields.postedAt = postedAt ? new Date(postedAt) : null;
    if (status !== undefined)
        updateFields.status = status;
    if (views !== undefined)
        updateFields.views = Number(views);
    if (likes !== undefined)
        updateFields.likes = Number(likes);
    if (comments !== undefined)
        updateFields.comments = Number(comments);
    if (shares !== undefined)
        updateFields.shares = Number(shares);
    ...
    try {
        ...
        for (const v of tiktokVideos) {
            ...
            const [existingVideo] = await db.select()
                .from(videos)
                .where(and(eq(videos.accountId, id), eq(videos.tiktokUrl, tiktokUrl)));
            if (existingVideo) {
                const [updatedVideo] = await db.update(videos)
                    .set({
