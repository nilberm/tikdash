 ...
 */
export async function fetchProfileMetrics(accessToken) {
    // 1. Tenta buscar todos os campos, incluindo stats (follower_count, likes_count)
    try {
        const response = await fetch('https://open.tiktokapis.com/v2/user/info/?fields=open_id,union_id,avatar_url,display_name,follower_count,likes_count,video_count', {
            headers: {
    ...
    try {
        const response = await fetch('https://open.tiktokapis.com/v2/user/info/?fields=open_id,union_id,avatar_url,display_name,video_count', {
            headers: {
                'Authorization': `Bearer ${accessToken}`,
            },
        });
 ...
 */
export async function fetchVideos(accessToken) {
    try {
        const response = await fetch('https://open.tiktokapis.com/v2/video/list/?fields=id,title,cover_image_url,create_time,like_count,comment_count,share_count,view_count,embed_link', {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${accessToken}`,
 ...
 */
export async function refreshTokenIfNeeded(accountId) {
